...
var LIBERTY_HOST="dal9-ppd-cwsicio2-01.sec.ibm.com";
...
var rootNodeElements = [
                    	{
                    		"displayText"	: "WebSEAL/RP Request",
                    		"shortText"		: "req"
                    	},
                    	{
                    		"displayText"	: "WebSEAL/RP Message",
                    		"shortText"		: "msg"
                    	},
                    	{
                    		"displayText"	: "ISAM/MGA Message",
                    		"shortText"		: "mes"
                    	}
...
$(document).ready(function() {
	...
	$('#dialog-download-test').dialog({
		...
		buttons: {
			"OK": function() {
				console.log($('#radio-dialog-retrieve-download-test').is(":checked"));
				if ($('#radio-dialog-retrieve-download-test').is(":checked")) {
					dKey = $('#text-dialog-retrieve-download-test').val();
					retrieveLogs("Test", $('#text-dialog-retrieve-download-test').val());
					$(this).dialog("close");				
					$('#dialog-please-wait').dialog("open");				
				} else {
		...
		for (var i = 0; i < STAG_HOSTS_WEBSEALS.length; i++) {
		  $('#jstree-div-stag-host-' + (i+1)).jstree(
				  {'plugins':["wholerow","checkbox"], 'core' : {
					  'data' : getJsTreeCheckboxes("Stag", i)
					   }});
		} // end for
	} else if (env.toLowerCase() == "Test".toLowerCase() || env.toLowerCase() == "Dev".toLowerCase()) {
		for (var i = 0; i < TEST_HOSTS_WEBSEALS.length; i++) {
			...
			success: function(data) {
				...
				if (typeof downloadKey != 'undefined' && downloadKey != "") {
					...
					if (typeof message != 'undefined' && message != "" && !message.includes(downloadKey)) {
						console.log("Values of downloadKey and message do not match");
//						displayMessage("Your download key is: " + downloadKey, message);
						displayMessage("Please refine your request based on details below.", message);
					} else {
						console.log("Values of downloadKey and message match, or is not accompanied with a message");
						displayDownloadKey(downloadKey);
					}
...
function retrieveLogs(env, downloadKey) {
	...
	if (env.toLowerCase() == "Staging".toLowerCase()) {
		...
		var RETRIEVE_ZIP_STAG_HOST = REST_HOST;
		
//		console.log("Calling " + STAG_HOST + "/w3id-svc/getWebSealLogs" + url);
		console.log("Calling " + RETRIEVE_ZIP_STAG_HOST + LOGS_FETCH_URI + "?" + url);
		
		$.ajax({
//			url: STAG_HOST + "/w3id-svc/getWebSealLogs",
			statusCode: {
				500: function() {
					displayMessage("Error encountered in retrieving downloaded files in ZIP format.", "Please look for " + downloadKey + ".zip file on " + LIBERTY_HOST);
				}
			},
			url: RETRIEVE_ZIP_STAG_HOST + LOGS_FETCH_URI + "?" + url,
			type: "get",
			async: true
//			data: url
			...
			if (xhr.getResponseHeader('Content-Type') == "application/json") {
				...
				window.open(RETRIEVE_ZIP_STAG_HOST + LOGS_FETCH_URI + "?" + url);
			}
		});; // end $.ajax
				
	} else if (env.toLowerCase() == "Dev".toLowerCase() || env.toLowerCase() == "Test".toLowerCase()) {

		var RETRIEVE_ZIP_TEST_HOST = REST_HOST;

//		console.log("Calling " + TEST_HOST + "/w3id-svc/getWebSealLogs" + url);
		console.log("Calling " + RETRIEVE_ZIP_TEST_HOST + LOGS_FETCH_URI + "?"  + url);
...
//				url: TEST_HOST + "/w3id-svc/getWebSealLogs",
				...
				url: RETRIEVE_ZIP_TEST_HOST + LOGS_FETCH_URI + "?" + url,
				type: "get",
				async: true
//				data: url
			}).done(function (data, textStatus, xhr) {
				if (xhr.getResponseHeader('Content-Type') == "application/json") {
					...
					$('#dialog-please-wait').dialog("close");
					displayMessage("File download is ready.", "Please ensure you allow pop-ups for this session.");
					window.open(RETRIEVE_ZIP_TEST_HOST + LOGS_FETCH_URI + "?" + url);
				}
			});; // end $.ajax
					
	} else if (env.toLowerCase() == "Prod".toLowerCase()) {

		var RETRIEVE_ZIP_PROD_HOST = REST_HOST;

//		console.log("Calling " + PROD_HOST + "/w3id-svc/getWebSealLogs" + url);
		console.log("Calling " + RETRIEVE_ZIP_PROD_HOST + LOGS_FETCH_URI + "?"  + url);
...
//			url: PROD_HOST + "/w3id-svc/getWebSealLogs",
			...
			url: RETRIEVE_ZIP_PROD_HOST + LOGS_FETCH_URI + "?" + url,
			type: "get",
			async: true
//			data: url
		}).done(function (data, textStatus, xhr) {
			if (xhr.getResponseHeader('Content-Type') == "application/json") {
				...
				$('#dialog-please-wait').dialog("close");
				displayMessage("File download is ready.", "Please ensure you allow pop-ups for this session.");
				window.open(RETRIEVE_ZIP_PROD_HOST + LOGS_FETCH_URI + "?" + url);
			}
		});; // end $.ajax		
...
function displayDownloadKey(downloadKey) {
	console.log("Inside displayDownloadKey(downloadKey)");
	console.log("Download key is " + downloadKey);
	$('#dialog-please-wait').dialog("close");
    $('#p-dialog-result-line1').text("Your download key is: " + downloadKey);
    $('#p-dialog-result-line2').text("You will be notifed by email once your log files are ready.");
...
function getJsTreeCheckboxes(env, iteration) {
	...
	for (var j = 0; j < rootNodeElements.length; j++) {
		if (rootNodeElements[j].shortText == "mes") { // this is due to the REST API change in ISAM 9
			continue;
		} else {
