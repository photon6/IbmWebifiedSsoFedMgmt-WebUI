...
//		});
	...
	$('#btn-browse-prod-rp-host-1').on('click', function(e) {
		fetchListing("Prod", "RP", "https://dal9-prd-cwsicio1-01a.sec.ibm.com", 1);
		$('#btn-selectall-prod-rp-host-1').prop('disabled', false);
	});
...
function fetchListing(env, type, host) {
	console.log("Initializing logs selection for " + env.toLowerCase() + " and type of " + type.toLowerCase() + "from host " + host + "...");
	
	if (env.toLowerCase() == "Staging".toLowerCase()) {
		if (type.toLowerCase() == "RP".toLowerCase()) {
...
//			for (var i = 0; i < PROD_HOSTS_WEBSEAL.length; i++) {
				...
				for (var j = 0; j < rpElements.length; j++) {
					...
					$('#jstree-div-prod-' + rpElements[j].serverType + '-host-' + (i+1)).jstree(
							{'plugins':["wholerow","checkbox"], 'core' : {
								'data' : getJsTreeCheckboxes(rpElements, "Prod", (i+1), false, 'https://dal9-prd-cwsicio1-01a.sec.ibm.com')
									}
							}
...
//			} // end for with i
		} // end if (type.toLowerCase() == "RP".toLowerCase()) {
...
function getJsTreeCheckboxes(elements, env, iteration, enabledVal, host) { // , shortText, displayText // <div id="jstree-div-prod-rp-host-4" class="jstree-div"></div>
	...
	for (var k = 0; k < elements.length; k++) {
		...
		jsTreeCheckboxesData.push({
			...
			"children"	: [{
			        						"id"		: "jstree-div-" + env.toLowerCase() + "-" + elements[k].serverType + "-host-" + elements[k].shortText + "-" + iteration+ "-" + (k+1),
			        					    "parent"	: "jstree-div-" + env.toLowerCase() + "-" + elements[k].serverType + "-host-" + elements[k].shortText + "-" + iteration,
			        					    "text"		: elements[k].displayText,
			        					    "state" 	: {
			        					    				opened 		: false,
			        					    				disabled	: false,
			        					     ...
			        					     "children"	: getLogsListing(env, host, elements[k].shortText)
			        		    			},
			        		    			{
				        						"id"		: "jstree-div-" + env.toLowerCase() + "-" + elements[k].serverType + "-host-" + elements[k].shortText + "-" + iteration+ "-" + (k+1) + "-child",
			        					        "parent"	: "jstree-div-" + env.toLowerCase() + "-" + elements[k].serverType + "-host-" + elements[k].shortText + "-" + iteration+ "-" + (k+1),
				        					    "text"		: elements[k].shortText,
			        					        "state" 	: {
			        					        				opened 		: false,
			        					        				disabled	: false,
			        					        				selected	: false
